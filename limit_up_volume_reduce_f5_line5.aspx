    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        */
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            double line5Price = KLine.GetAverageSettlePrice(stock.kLineDay, currentIndex, 5, 5);
            if (line3Price <= line5Price)
            {
                continue;
            }

            if (stock.kLineDay[currentIndex].lowestPrice >= line5Price * 1.01)
            {
                continue;
            }
            ...
            //buyPrice = Math.Max(stock.kLineDay[currentIndex].lowestPrice, line5Price) * 1.02 ;

            buyPrice = stock.kLineDay[currentIndex].endPrice;
            if (buyPrice <= line5Price)
            {
                continue;
            }
            ...
            double currentCrossPrice = 0;
            double previousCrossPrice = 0;

            bool isGoldCross = true;

            for (int i = currentIndex; i > 0 && (currentCrossPrice == 0 || previousCrossPrice == 0); i--)
            {
                if (isGoldCross
                    && stock.GetAverageSettlePrice(i - 1, 3, 3) <= stock.GetAverageSettlePrice(i - 1, 5, 5))
                {
                    currentCrossPrice = stock.GetAverageSettlePrice(i, 5, 5);
                    isGoldCross = false;
                }
                if (!isGoldCross
                    && stock.GetAverageSettlePrice(i - 1, 3, 3) >= stock.GetAverageSettlePrice(i - 1, 5, 5))
                {
                    previousCrossPrice = stock.GetAverageSettlePrice(i, 5, 5);
                }
            }
            if (currentCrossPrice <= previousCrossPrice)
            {
                continue;
            }
