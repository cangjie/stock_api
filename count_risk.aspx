...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        dt.Columns.Add("总计");
        DataTable dtOri = DBHelper.GetDataTable(" select * from risk d1 where risk < 20 and exists ( select 'a' from risk d2 where d1.gid = d2.gid and d2.risk - d1.risk >= 30 and d1.alert_date = dbo.func_GetLastTransactDate(d2.alert_date, 1) ) "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            int buyIndex = 0;
            for (int i = alertIndex; i < s.kLineDay.Length; i++)
            {
            ...
            if (buyIndex <= 0 || buyIndex > s.kLineDay.Length - days)
            {
                continue;
            }
            int kdjWeekIndex = s.GetItemIndex(s.kLineDay[buyIndex].endDateTime, "week");

            if (s.kdjWeeks(kdjWeekIndex) < 0)
            {
                continue;
            }
