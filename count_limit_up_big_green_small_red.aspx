...
<script runat="server">
    ...
    public DataTable GetData(string countPage)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            for (int i = alertIndex + 2; i < s.kLineDay.Length && i <= alertIndex + 11; i++)
            {
                if (s.kLineDay[i].startPrice < s.kLineDay[i].endPrice
                    && (s.kLineDay[i - 1].endPrice - s.kLineDay[i - 1].startPrice) / s.kLineDay[i - 1].startPrice <= -0.05
                    && s.kLineDay[i].volume / s.kLineDay[i - 1].volume <= 0.4)
                {
                    buyIndex = i;
                    break;
                }
            ...
            if ((s.kLineDay[buyIndex].endPrice - s.kLineDay[buyIndex - 1].endPrice) / s.kLineDay[buyIndex - 1].endPrice >= 0.05)
            {
                continue;
            }

            if (s.kLineDay[buyIndex].endPrice < s.GetAverageSettlePrice(buyIndex, 20, 0))
            {
                //continue;
            }
