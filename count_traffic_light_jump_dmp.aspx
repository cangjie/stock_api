...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            int buyIndex = -1;
            for (int i = 1;
                i <= maxDays && s.kLineDay.Length > alertIndex + i
                && s.kLineDay[alertIndex + i].highestPrice < s.kLineDay[alertIndex].endPrice;
                i++)
            {
                if (s.kLineDay[alertIndex + i].startPrice > s.kLineDay[alertIndex + i - 1].endPrice 
                    && s.kLineDay[alertIndex + i].startPrice < s.kLineDay[alertIndex + i].endPrice)
                {
                    if (s.kLineDay[alertIndex + i].macd > 0 && s.kLineDay[alertIndex + i - 1].macd < 0)
                    {
                        double dmpPrice = KLine.ComputeDMP(s.kLineDay, alertIndex + i);
                        if (s.kLineDay[alertIndex + i].startPrice > dmpPrice)
                        {
