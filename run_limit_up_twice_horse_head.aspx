...
<script runat="server">
    ...
    public int over1Per = 0;

    public int over5Per = 0;

    public int over10Per = 0;
    ...
    public DataTable GetData()
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (stock.kLineDay[currentIndex].highestPrice > stock.kLineDay[currentIndex + 1].lowestPrice)
            {
                continue;
            }
...
<body>
    ...
    <div>
        超10%：<%=Math.Round(100 * (double)over10Per / (double)count, 2).ToString() %>%<br />
        超5%：<%=Math.Round(100 * (double)over5Per / (double)count, 2).ToString() %>%<br />
        超1%：<%=Math.Round(100 * (double)over1Per / (double)count, 2).ToString() %>%<br />
    </div>
    <div>
        <div>总计：<%=count %></div>
