...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select * from limit_up where alert_date = '" + lastTransactDate.ToShortDateString() + "' "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            */
            ...
            if (stock.kLineDay[currentIndex].startPrice <= stock.kLineDay[alertIndex].endPrice)
            {
                continue;
            }

            if (alertIndex < 2 || alertIndex >= stock.kLineDay.Length - 1)
            {
                continue;
            }

            KLine k = stock.kLineDay[alertIndex];
            ...
            if (Math.Abs((k.endPrice - k.startPrice) - (k.startPrice - k.lowestPrice)) / (k.startPrice - k.lowestPrice) > 0.3)
            {
                continue;
            }
            ...
            int limitUpIndex = alertIndex;



            double hightestPrice = Math.Max(stock.kLineDay[limitUpIndex].highestPrice, stock.kLineDay[limitUpIndex + 1].highestPrice);
            hightestPrice = Math.Max(hightestPrice, stock.kLineDay[alertIndex].highestPrice);
            int lowestIndex = -1;
            ...
            for (int i = 1; i <= 20 && alertIndex + i <  currentIndex; i++)
            {
                ...
                if (stock.kLineDay[alertIndex + i].lowestPrice <= f5 * 1.01)
                {
                    if (f5Index == 0)
                    {
                        f5Index = alertIndex + i;
                        break;
                    }
                }
                else if (stock.kLineDay[alertIndex + i].lowestPrice <= f3 * 1.01)
                {
            ...
            dr["调整"] = 0;
            dr["缩量"] = (stock.kLineDay[currentIndex].volume - stock.kLineDay[currentIndex - 1].volume) / stock.kLineDay[currentIndex - 1].volume;
            dr["现高"] = hightestPrice;
            dr["F3"] = f3;
            dr["F5"] = f5;
            dr["前低"] = lowestPrice;
            ...
            dr["涨幅"] = (stock.kLineDay[currentIndex].endPrice - stock.kLineDay[currentIndex - 1].endPrice) / stock.kLineDay[currentIndex - 1].endPrice;
