...
<script runat="server">
    ...
    public DataTable GetData(string countPage)
    {
        ...
        dt.Columns.Add("总计");
        DataTable dtOri = DBHelper.GetDataTable(" select * from alert_volume_equal where is_limit_up = 0  and alert_date >= '"
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (!s.IsLimitUp(alertIndex - 1) || Math.Abs(s.kLineDay[alertIndex].volume - s.kLineDay[alertIndex - 1].volume)/s.kLineDay[alertIndex - 1].volume > 0.1 )
            {
                continue;
            }
            ...
            for (int i = alertIndex + 1; i < s.kLineDay.Length || i <= alertIndex + 10 ; i++)
            {
                double currentVolume = s.kLineDay[i].volume;
                double lastVolume = s.kLineDay[i - 1].volume;
                if (currentVolume / lastVolume <= 0.4)
                {
                    buyIndex = i;
                    break;
                }
            }
            if (buyIndex == 0 || buyIndex + days >= s.kLineDay.Length || s.IsLimitUp(buyIndex))
            {
                continue;
            }
