    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            */
            ...
            stock.kLineQuaterHour = Stock.LoadRedisKLine(stock.gid, "30min", Util.rc);

            DateTime currentTime = currentDate.Date;
            if (currentTime == DateTime.Now.Date)
            {
                if (DateTime.Now < DateTime.Now.Date.AddHours(9).AddMinutes(30))
                {
                    currentTime = currentTime.AddDays(-1).AddHours(15);
                }
                else if (DateTime.Now < DateTime.Now.Date.AddHours(11).AddMinutes(30))
                {
                    currentTime = DateTime.Now;
                }
                else if (DateTime.Now < DateTime.Now.Date.AddHours(13))
                {
                    currentTime = DateTime.Now.AddHours(11).AddMinutes(30);
                }
                else if (DateTime.Now < DateTime.Now.Date.AddHours(15))
                {
                    currentTime = DateTime.Now;
                }
                else
                {
                    currentTime = DateTime.Now.Date.AddHours(15);
                }
            }
            else
            {
                currentTime = currentDate.Date.AddHours(15);
            }
            ...
            double pressureMa = double.MaxValue;
            double supportMa = 0;
            KeyValuePair<string, double>[] currentQuota = stock.GetSortedQuota(currentIndex);
            for (int i = currentQuota.Length - 1; i >= 0; i--)
            {
                if (currentQuota[i].Key.StartsWith("ma"))
                {
                    if (openPrice > currentQuota[i].Value && supportMa == 0)
                    {
                        supportMa = currentQuota[i].Value;
                    }
                    if (openPrice < currentQuota[i].Value)
                    {
                        pressureMa = currentQuota[i].Value;
                    }
                }
            }
            int highIndex = 0;
            ...
            */
            KeyValuePair<string, double>[] sortedSupportArr = new KeyValuePair<string, double>[4];
            sortedSupportArr[0] = new KeyValuePair<string, double>("settle", settlePrice);
            sortedSupportArr[1] = new KeyValuePair<string, double>("ma", supportMa);
            sortedSupportArr[2] = new KeyValuePair<string, double>("f3", f3);
            sortedSupportArr[3] = new KeyValuePair<string, double>("f5", f5);
            sortedSupportArr = Core.Util.SortKeyValuePairDesc(sortedSupportArr);
            ...
            */
            ...
            bool isUpFoot = false; //UpFoot(timelineArray, out todayHighestPrice, out todayDisplayHighPrice, out upFootTime);
