...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        double coverRate = double.Parse(Util.GetSafeRequestValue(Request, "cover", "0").Trim());
        DataTable dtOri = GetData(currentDate, coverRate);
    ...
    public static DataTable GetData(DateTime currentDate, double coverRate)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (!stock.IsLimitUp(trafficLightIndex - 2))
            {
                continue;
            }
            ...
            {   
                ...
                if ((stock.kLineDay[i - 1].lowestPrice < f5 || stock.kLineDay[i].lowestPrice < f5)
                    && stock.kLineDay[i].endPrice > f5 && stock.kLineDay[i].startPrice < stock.kLineDay[i].endPrice)
                {
                    if (f5Index == 0)
                    {
                        f5Index = i;
                ...
                else if ((stock.kLineDay[i - 1].lowestPrice < f3 || stock.kLineDay[i].lowestPrice < f3)
                    && stock.kLineDay[i].endPrice > f3 && stock.kLineDay[i].startPrice < stock.kLineDay[i].endPrice)
                {
                    if (f3Index == 0)
                    {
                        f3Index = i;
            ...
            if (f5Index == 0 && f3Index == currentIndex)
            {
                reversePoint = "F3";
            }
            if (reversePoint.Trim().Equals(""))
            {
                continue;
            }
            DataRow dr = dt.NewRow();
