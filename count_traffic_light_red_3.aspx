...
<script runat="server">
    ...
    public DataTable GetData(string countPage)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            int limitupIndex = s.GetItemIndex(DateTime.Parse(drOri["alert_date"].ToString()));
            if (limitupIndex >= s.kLineDay.Length - 5)
            {
                continue;
            }
            if (s.kLineDay[limitupIndex + 1].endPrice >= s.kLineDay[limitupIndex + 1].startPrice)
            {
                continue;
            }
            if (s.kLineDay[limitupIndex + 2].endPrice <= s.kLineDay[limitupIndex + 2].startPrice)
            {
                continue;
            }
            if (s.kLineDay[limitupIndex + 3].endPrice <= s.kLineDay[limitupIndex + 3].startPrice)
            {
                continue;
            }
            if (s.kLineDay[limitupIndex + 4].endPrice <= s.kLineDay[limitupIndex + 4].startPrice)
            {
                continue;
            }
            if (s.kLineDay[limitupIndex + 5].endPrice <= s.kLineDay[limitupIndex + 5].startPrice)
            {
                continue;
            }


            int buyIndex = limitupIndex + 5 ;
