...
<script runat="server">
    ...
    public DataTable GetData(string countPage)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (Math.Abs(s.kLineDay[alertIndex - 2].volume - s.kLineDay[alertIndex - 1].volume) / s.kLineDay[alertIndex].volume >= 0.1)
            {
                continue;
            }

            if (s.kLineDay[alertIndex].volume <= Math.Max(s.kLineDay[alertIndex - 1].volume, s.kLineDay[alertIndex - 2].volume))
            {
                continue;
            }

            if (s.kLineDay[alertIndex].highestPrice <= Math.Max(s.kLineDay[alertIndex - 1].highestPrice, s.kLineDay[alertIndex - 2].highestPrice))
            {
                continue;
            }
