...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        dt.Columns.Add("无影价", Type.GetType("System.Double"));
        ...
        foreach (DataRow drOri in dtIOVolume.Rows)
        {
            ...
            for (int i = startIndex; i < currentIndex; i++)
            {
                ...
                {
                    lowest = stock.kLineDay[i].lowestPrice;
                    lowestIndex = i;
                }
            ...
            if (highIndex <= lowestIndex)
            {
                continue;
            }
            double widthRate = (highest - lowest) / lowest;
            if (widthRate <= 0.2)
            {
                continue;
            }
            ...
            double volumeYesterday = 100;// stock.kLineDay[limitUpIndex].volume;// Stock.GetVolumeAndAmount(stock.gid, DateTime.Parse(stock.kLineDay[limitUpIndex].startDateTime.ToShortDateString() + " " + DateTime.Now.Hour.ToString() + ":" + DateTime.Now.Minute.ToString()))[0];
            /*
            ...
            */
            ...
            dr["评级"] = memo;
            buyPrice = stock.kLineDay[currentIndex].endPrice;//double.Parse(drOri["foot_price"].ToString()) * (1 + double.Parse(drOri["foot_rate"].ToString()));
            dr["买入"] = buyPrice;
