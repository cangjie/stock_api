...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        DataTable dtLimitUp = DBHelper.GetDataTable(" select * from limit_up where alert_date <= '"
            + Util.GetLastTransactDate(DateTime.Now.Date, 12) + "' order by alert_date desc ");
        foreach (DataRow drLimitUp in dtLimitUp.Rows)
        {
            ...
            if (!SearchLimitUp(s.kLineDay, limitUpIndex))
            {
                continue;
            }
            double currentSettlePrice = s.kLineDay[limitUpIndex + 1].endPrice;
            double buyPrice = s.kLineDay[limitUpIndex + 1].startPrice;

            s.kLineDay[limitUpIndex + 1].endPrice = buyPrice;
            KLine.ComputeMACD(s.kLineDay);
            if (s.kLineDay[limitUpIndex].macd >= 0 || s.kLineDay[limitUpIndex + 1].macd <= 0 || buyPrice <= s.kLineDay[limitUpIndex].highestPrice)
            {
                continue;
            }
            s.kLineDay[limitUpIndex + 1].endPrice = currentSettlePrice;
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                dr[(i + 1).ToString() + "日"] = (s.kLineDay[limitUpIndex + 1 + 1 + i].highestPrice - buyPrice) / buyPrice;
            }
