...
<script runat="server">
    ...
    public DataTable GetData(string countPage)
    {
        ...
        dt.Columns.Add("10日");
        dt.Columns.Add("总计");
        DataTable dtOri = DBHelper.GetDataTable(" select * from alert_traffic_light  a where exists ( "
            + "  select 'a' from alert_traffic_light b where b.alert_date = dbo.func_GetLastTransactDate(a.alert_date, 3) and a.gid = b.gid )" );
        foreach (DataRow drOri in dtOri.Rows)
        {

            //bool newHigh = false;
            Stock s = GetStock(drOri["gid"].ToString().Trim());
            int alertIndex = s.GetItemIndex(DateTime.Parse(drOri["alert_date"].ToString())) - 2;


            if (alertIndex < 3 || alertIndex + 2 >= s.kLineDay.Length)
            {
                continue;
            }
            ...
            if (!s.IsLimitUp(alertIndex)
                || s.kLineDay[alertIndex + 1].startPrice <= s.kLineDay[alertIndex + 1].endPrice
                || s.kLineDay[alertIndex + 2].endPrice <= s.kLineDay[alertIndex + 2].startPrice
                || s.kLineDay[alertIndex - 1].startPrice >= s.kLineDay[alertIndex - 1].endPrice
                || s.kLineDay[alertIndex - 2].endPrice >= s.kLineDay[alertIndex - 2].startPrice
                || !s.IsLimitUp(alertIndex - 3))
            {
                continue;
            }
