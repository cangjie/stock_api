...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select gid, alert_date from alert_traffic_light a where  alert_date <= '" + Util.GetLastTransactDate(currentDate, 1).ToShortDateString() + "' "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            for (int i = alertIndex - 2; i < stock.kLineDay.Length && i < currentIndex; i++)
            {
                if (stock.kLineDay[i].highestPrice > stock.kLineDay[currentIndex].endPrice)
                {
                    isHighest = false;
                    break;
                }
            ...
            int highIndex = 0;
            int lowestIndex = 0;
            double lowest = GetFirstLowestPrice(stock.kLineDay, alertIndex, out lowestIndex);
            double highest = 0;
            ...
            double maxVolume = stock.kLineDay[alertIndex].volume;
            ...
            */
            ...
            if (alertIndex + 1 < stock.kLineDay.Length)
            {
                volumeToday = stock.kLineDay[alertIndex+1].volume;
                if(stock.kLineDay[alertIndex+1].endDateTime.Date == DateTime.Now.Date && DateTime.Now.Hour < 15)
                {
                    volumeToday = stock.kLineDay[alertIndex+1].VirtualVolume;
                }
            }

            double volumeYesterday = stock.kLineDay[alertIndex].volume;// Stock.GetVolumeAndAmount(stock.gid, DateTime.Parse(stock.kLineDay[limitUpIndex].startDateTime.ToShortDateString() + " " + DateTime.Now.Hour.ToString() + ":" + DateTime.Now.Minute.ToString()))[0];
            /*
            ...
            */
            ...
            dr["调整"] = currentIndex - alertIndex - 1;
            dr["缩量"] = volumeReduce;
            dr["现高"] = highest;
            dr["F3"] = f3;
            dr["F5"] = f5;
