...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        DataTable dtGreens = DBHelper.GetDataTable(" select * from alert_continuous_green where alert_date >= '" + lastTransactDate.ToShortDateString()
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            int currentIndex = stock.GetItemIndex(currentDate.Date);
            ...
            double avaVolume = Stock.GetAvarageVolume(stock.kLineDay, currentIndex, 10);
            if (stock.kLineDay[currentIndex].volume <=  avaVolume)
            {
                continue;
            }
            ...
            int currentWeekIndex = stock.GetItemIndex(currentDate.Date, "week");
            ...
            DataRow[] drGreenArr = dtGreens.Select(" gid = '" + stock.gid.Trim() + "' ", " alert_date desc ");
            if (drGreenArr.Length <= 0)
            {
                continue;
            }
            DateTime greenDate = DateTime.Parse(drGreenArr[0]["alert_date"].ToString());
            int greenIndex = stock.GetItemIndex(greenDate);
            if (greenIndex < 0 || greenIndex >= stock.kLineDay.Length)
            {
                continue;
            }
            int buyIndex = -1;
            ...
            dr["放量"] = (stock.kLineDay[currentIndex].volume - avaVolume) / avaVolume;
