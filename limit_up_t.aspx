...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select gid, alert_date from limit_up where alert_date = '" + Util.GetLastTransactDate(currentDate, 1).ToShortDateString()
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (stock.gid.Trim().Equals("sz000702"))
            {
                string aa = "aa";
            }
            ...
            if (!(stock.kLineDay[currentIndex - 2].macd <= 0 && stock.kLineDay[currentIndex - 1].macd >= 0))
            {
                //continue;
            }
            if ((stock.kLineDay[currentIndex].startPrice - stock.kLineDay[currentIndex - 1].endPrice) / stock.kLineDay[currentIndex - 1].endPrice < 0.099)
            {
                continue;
            }
