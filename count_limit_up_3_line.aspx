...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select a.alert_date, a.gid from limit_up a "
            + " where not exists( select 'a' from limit_up b where a.gid = b.gid and dbo.func_GetNextTransactDate(a.alert_date, 1) = b.alert_date) "
            + " and not exists ( select 'a' from limit_up c where a.gid = c.gid and dbo.func_GetNextTransactDate(a.alert_date, 2) = c.alert_date )  "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            double lowestPrice = s.kLineDay[currentIndex + 2].lowestPrice;
            double line3Price = s.GetAverageSettlePrice(currentIndex + 2, 3, 3);
            if (Math.Abs(lowestPrice - line3Price) / line3Price > 0.005)
            {
                continue;
            }
