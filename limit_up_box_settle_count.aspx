...
<script runat="server">
    ...
    {
        for (DateTime i = DateTime.Parse("2018-4-1"); i <= DateTime.Parse("2018-9-30"); i = i.AddDays(1))
        {
                    ...
                    /*
                        try
                        {
                            DBHelper.InsertData("bread_count", new string[,] {
                                ...
                                {"gid", "varchar", dr["代码"].ToString() },
                                {"volume_reduce", "float", dr["缩量"].ToString() },
                                {"macd", "int", dr["MACD日"].ToString() },
                                {"kdj", "int", dr["KDJ日"].ToString() },
                                {"adjust_days", "int", dr["调整"].ToString() },
                                {"line3", "float", dr["3线"].ToString() },
                                {"highest", "float", dr["现高"].ToString() },
                                {"f3", "float", dr["F3"].ToString() },
                                {"f5", "float", dr["F5"].ToString() },
                                {"lowest", "float", dr["前低"].ToString() },
                                {"width", "float", (Math.Round(double.Parse(dr["幅度"].ToString().Replace("%", "")), 2)/100).ToString() },
                                {"support_type", "varchar", dr["类型"].ToString() },
                                {"price_diff", "float", dr["价差"].ToString() },
                                {"buy_price", "float", dr["买入"].ToString() },
                                {"top_price_rate", "float", dr["总计"].ToString() }
