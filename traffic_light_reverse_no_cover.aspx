...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate, int days)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            {
                //trafficLightIndex = stock.GetItemIndex(DateTime.Parse(drOri["alert_date"].ToString()));
                continue;
            }

            string color = "";

            if (Math.Abs(stock.kLineDay[trafficLightIndex - 1].volume - stock.kLineDay[trafficLightIndex - 2].volume) / stock.kLineDay[trafficLightIndex - 2].volume < 0.02)
            {
                color = "green";
            }
            else if (stock.kLineDay[trafficLightIndex].volume > stock.kLineDay[trafficLightIndex - 1].volume
                && stock.kLineDay[trafficLightIndex].highestPrice > stock.kLineDay[trafficLightIndex - 1].highestPrice)
            {
                color = "red";
            }
            ...
            {
                case "red":
                    dr["信号"] = dr["信号"].ToString() + "<a title=\"红灯量价反包\" >🔴</a>";
                    break;
                case "green":
