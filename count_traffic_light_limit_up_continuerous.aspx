...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        dt.Columns.Add("总计");
        DataTable dtOri = DBHelper.GetDataTable(" select * from alert_traffic_light where  "
            + " exists ( select 'a' from limit_up a where a.alert_date = dbo.func_GetLastTransactDate(alert_traffic_light.alert_date, 2) and a.gid = alert_traffic_light.gid) "
            + " and exists ( select 'a' from limit_up b where b.alert_date = dbo.func_GetLastTransactDate(alert_traffic_light.alert_date, 3) and b.gid = alert_traffic_light.gid )  "
            + " and exists ( select 'a' from limit_up d where d.alert_date = alert_traffic_light.alert_date and d.gid = alert_traffic_light.gid ) "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            bool gap = Util.GetSafeRequestValue(Request, "gap", "1").Trim().Equals("0") ? true : false;
            if (gap && s.kLineDay[buyIndex].lowestPrice <= s.kLineDay[buyIndex-1].highestPrice)
            {
                continue;
