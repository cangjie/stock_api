...
<script runat="server">
    ...
    {
        Search15minBuyPoint(currentDate);
        ...
        //DataTable dtDtl = DBHelper.GetDataTable(" select gid, alert_time, price from alert_foot where alert_date > '"
        //    + currentDate.ToShortDateString() + "' and alert_date < '" + currentDate.AddDays(1).ToShortDateString() + "'  order by alert_date desc ");

        DataTable dtOri = DBHelper.GetDataTable(" select distinct gid,  buy_price from alert_organize_buy_point where alert_time >= '" + currentDate.ToShortDateString()
            + "' and alert_time < '" + currentDate.AddDays(1).ToShortDateString() + "'   ");
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            */
            ...
            if (kdjDays > macdDays && kdjDays - macdDays > 2)
            {
                continue;
            }
            dr["KDJ日"] = kdjDays;
    ...
    public static void Search15minBuyPoint(DateTime currentDate)
    {
        DateTime startDate = Util.GetLastTransactDate(currentDate, 5);
        DataTable dtPromote = DBHelper.GetDataTable(" select * from organize_promote where promote_date >= '" + currentDate.ToShortDateString() + "' and promote_date <= '" + currentDate.ToShortDateString() + "' ");
        foreach (DataRow dr in dtPromote.Rows)
        {
            ...
            KLine.ComputeRSV(stock.kLineQuaterHour);
            KLine.ComputeKDJ(stock.kLineQuaterHour);
            int currentIndex = stock.GetItemIndex(currentDate);
            if (currentIndex > 0)
            {
                if (stock.kLineDay[currentIndex].macd > 0 && stock.kLineDay[currentIndex].j >= stock.kLineDay[currentIndex].d)
                {
                    for (int i = stock.kLineQuaterHour.Length - 1; i >= 0; i--)
                    {
                        if (stock.kLineQuaterHour[i].endDateTime.ToShortDateString().Equals(currentDate.ToShortDateString()) && i > 0)
                        {
                            if (stock.kLineQuaterHour[i].macd > 0 && stock.kLineQuaterHour[i - 1].macd < 0)
                            {
                                try
                                {
                                    DBHelper.InsertData("alert_organize_buy_point", new string[,] {
                                    {"alert_time", "datetime", stock.kLineQuaterHour[i].endDateTime.ToShortDateString() + " " + stock.kLineQuaterHour[i].endDateTime.ToShortTimeString() },
                                    {"alert_type", "varchar", "macd" },
                                    {"gid", "varchar", stock.gid.Trim() },
                                    {"buy_price", "float", stock.kLineQuaterHour[i].endPrice.ToString() }
                                    });
                                }
                                catch
                                {
                            ...
                            if (stock.kLineQuaterHour[i].macd > 0 && stock.kLineQuaterHour[i].j >= stock.kLineQuaterHour[i].d && stock.kLineQuaterHour[i - 1].j <= stock.kLineQuaterHour[i - 1].d)
                            {
                                try
                                {
                                    DBHelper.InsertData("alert_organize_buy_point", new string[,] {
                                    {"alert_time", "datetime", stock.kLineQuaterHour[i].endDateTime.ToShortDateString() + " " + stock.kLineQuaterHour[i].endDateTime.ToShortTimeString() },
                                    {"alert_type", "varchar", "kdj" },
                                    {"gid", "varchar", stock.gid.Trim() },
                                    {"buy_price", "float", stock.kLineQuaterHour[i].endPrice.ToString() }
                                    });
                                }
