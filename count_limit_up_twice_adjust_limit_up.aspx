...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        dt.Columns.Add("总计");
        DataTable dtOri = DBHelper.GetDataTable(" select * from limit_up a where not exists (select 'a' from limit_up b where b.alert_date = dbo.func_GetLastTransactDate(a.alert_date, 1) and a.gid = b.gid) "
            + " and exists (select 'a' from limit_up c where c.alert_date = dbo.func_GetLastTransactDate(a.alert_date, 2) and a.gid = c.gid ) "
            + " and exists (select 'a' from limit_up d where d.alert_date = dbo.func_GetLastTransactDate(a.alert_date, 3) and a.gid = d.gid ) "


            + " and not exists (select 'a' from limit_up e where e.alert_date = dbo.func_GetLastTransactDate(a.alert_date, 4) and a.gid = e.gid ) "
            + " and not exists (select 'a' from limit_up f where f.alert_date = dbo.func_GetLastTransactDate(a.alert_date, 5) and a.gid = f.gid ) "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (!s.IsLimitUp(alertIndex) || s.IsLimitUp(alertIndex - 1) || !s.IsLimitUp(alertIndex - 2) || !s.IsLimitUp(alertIndex - 3))
            {
                continue;
            }

            if (s.kLineDay[alertIndex - 1].endPrice >= s.kLineDay[alertIndex - 1].startPrice)
            {
                continue;
            }
