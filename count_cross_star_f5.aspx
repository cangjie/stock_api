...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            int lowIndex = 0;
            double lowPrice = GetFirstLowestPrice(s.kLineDay, currentIndex - 1, out lowIndex);
            if (lowIndex == 0)
            {
                continue;
            }
            double f5 = 0.382 * (s.kLineDay[currentIndex].highestPrice - lowPrice) + lowPrice;

            if (s.kLineDay[currentIndex + 1].lowestPrice > (f5 + 0.02) || s.kLineDay[currentIndex + 1].endPrice < f5)
            {
                continue;
            }
            ...
            for (int i = 1; i <= 5; i++)
            {
                double rate = (s.kLineDay[currentIndex + 1 + i].highestPrice - buyPrice) / buyPrice;
...
<body>
    ...
    <div>
        <%=success.ToString() %> / <%=count.ToString() %> = <%=Math.Round(100 * (double)success/count, 2).ToString() %>%<br />
