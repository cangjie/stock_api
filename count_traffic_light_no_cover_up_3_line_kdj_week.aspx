...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        int lastDays = int.Parse(Util.GetSafeRequestValue(Request, "lastdays", "40"));
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            int buyIndex = 0;
            for (int i = 1; i <= lastDays && alertIndex + i < s.kLineDay.Length
                && s.kLineDay[alertIndex + i].highestPrice < s.kLineDay[alertIndex].endPrice; i++)
            {
                if ((s.kLineDay[alertIndex + i].endPrice > s.GetAverageSettlePrice(alertIndex + i, 3, 3)
                    || (s.kLineDay[alertIndex + i].startPrice < s.GetAverageSettlePrice(alertIndex + i, 3, 3)
                    && s.kLineDay[alertIndex + i - 1].endPrice <  s.GetAverageSettlePrice(alertIndex + i - 1, 3, 3)) )
                    )
                {
            ...
            if (s.kLineDay[buyIndex].endPrice <= s.kLineDay[buyIndex - 1].endPrice)
            {
                continue;
            }

            DateTime buyDate = s.kLineDay[buyIndex].startDateTime.Date;

            int weekIndex = s.GetItemIndex(buyDate, "week");
