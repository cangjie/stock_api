...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate, int days)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            int alertIndex = stock.GetItemIndex(alertDate);

            if (alertIndex < 0 || alertIndex >= stock.kLineDay.Length-1)
            {
                continue;
            }

            if (!stock.IsLimitUp(alertIndex))
            {
                continue;
            }

            if (Math.Abs(stock.kLineDay[alertIndex + 1].volume - stock.kLineDay[alertIndex].volume) / stock.kLineDay[alertIndex].volume > 0.1)
            {
                continue;
            }
            ...
            */
            ...
            double width = Math.Round(100 * (highestPrice - lowestPrice) / lowestPrice, 2);




            KLine highKLine = stock.kLineDay[highestIndex];
            ...
            double prevHighPrice = Math.Max(stock.kLineDay[alertIndex + 1].highestPrice, stock.kLineDay[alertIndex].highestPrice);

            for (int i = alertIndex + 1; i <= alertIndex + 5 && i < stock.kLineDay.Length; i++)
            {
                if (stock.kLineDay[i].endPrice > prevHighPrice)
                { 
                    dr["信号"] = dr["信号"].ToString().Trim() + "<a title=\"新高\" >⛰️</a>";
                    break;
                }
            }

            if (Math.Min(stock.kLineDay[alertIndex + 1].startPrice, stock.kLineDay[alertIndex + 1].endPrice) > stock.kLineDay[alertIndex].highestPrice)
            {
                dr["信号"] = dr["信号"].ToString().Trim() + "<a title=\"剑鞘\" >🗡</a>";
            }
