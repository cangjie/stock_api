...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable("select * from limit_up where alert_date >= '" + Util.GetLastTransactDate(currentDate, 7).ToShortDateString() + "' and alert_date < '" + currentDate.ToShortDateString() + "' ");
        ...
        */
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (100*(stock.kLineDay[limitUpIndex + 1].volume  -  stock.kLineDay[limitUpIndex].volume)/ stock.kLineDay[limitUpIndex].volume  > rate )
            {
                continue;
            }

            if (limitUpIndex >= currentIndex )
            {
                continue;
            }
            ...
            {
                if (stock.kLineDay[i].highestPrice
                    > Math.Max(stock.kLineDay[limitUpIndex].highestPrice, stock.kLineDay[limitUpIndex+1].highestPrice))
                {
                    newHighIndex = i;
                    break;
                }
            ...
            */
            ...
            double volumeReduce = stock.kLineDay[currentIndex - 1].volume / stock.kLineDay[currentIndex - 2].volume;
