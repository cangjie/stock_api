...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate, double coverRate, double startLightRate, double endLightRate, bool showAll)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable("select * from limit_up a where alert_date = '" + Util.GetLastTransactDate(currentDate, 1).ToShortDateString()
            + "' and exists ( select 'a' from limit_up b where a.gid = b.gid and b.alert_date < dbo.func_GetLastTransactDate(a.alert_date, 1) and b.alert_date > dbo.func_GetLastTransactDate(a.alert_date, 20)  ) "
            //+ " and gid = 'sz000558' "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            int highestIndex = currentIndex - 1;
            double highestPrice = stock.kLineDay[highestIndex].highestPrice;
            int i = highestIndex - 1;
            for (; !stock.IsLimitUp(i) && i >= 0; i--)
            {
            ...
            if (highestIndex == currentIndex - 1)
            {
                continue;
            }
            int lowestIndex = -1;
            ...
            double f5UpLimit = f5 * 1.01;
            double f5DownLimit = f5 * 0.99;

            bool crossF5 = false;

            if (stock.kLineDay[currentIndex - 1].lowestPrice <= f5UpLimit
                || stock.kLineDay[currentIndex - 1].highestPrice >= f5DownLimit
                || (stock.kLineDay[currentIndex - 1].lowestPrice >= f5UpLimit && stock.kLineDay[currentIndex - 2].endPrice <= f5UpLimit))
            {
                crossF5 = true;
            }
