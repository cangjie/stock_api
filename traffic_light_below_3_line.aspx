...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (stock.kLineDay[alertIndex].endPrice <= stock.GetAverageSettlePrice(alertIndex, 3, 3))
            {
                continue;
            }
            ...
            int below3LineDays = 0;

            int lastBelow3LineIndex = -1;

            for (int i = alertIndex - 2; i < currentIndex && !(below3Line && stock.kLineDay[i].endPrice >= stock.GetAverageSettlePrice(i, 3, 3)) ; i++)
            {
                if (!below3Line && stock.kLineDay[i].endPrice < stock.GetAverageSettlePrice(i, 3, 3))
                {
                    below3Line = true;
                }
                if (below3Line)
