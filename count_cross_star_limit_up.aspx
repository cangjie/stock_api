...
<script runat="server">
    ...
    public static int touchSuc = 0;

    public static int unTouchCount = 0;

    public static int unTouchSuc = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if ((s.kLineDay[currentIndex].highestPrice - s.kLineDay[currentIndex - 1].endPrice) / s.kLineDay[currentIndex - 1].endPrice >= 0.0985)
            {
                touchCount++;
                if (maxRate >= 0.02)
                {
...
<body>
    ...
    <div>
        摸板： <%=touchSuc.ToString() %>/<%=touchCount %> = <%=Math.Round(100*(double)touchSuc/touchCount, 2).ToString() %>%<br />
        不摸： <%=unTouchSuc.ToString() %>/<%=unTouchCount %> = <%=Math.Round(100*(double)unTouchSuc/unTouchCount, 2).ToString() %>%<br />
