...
<script runat="server">
    ...
    public DataTable GetData(string countPage)
    {
        ...
        dt.Columns.Add("总计");
        DataTable dtOri = DBHelper.GetDataTable(" select * from limit_up a where alert_date >= '"
            + Util.GetSafeRequestValue(Request, "start", "2021-1-1") + "'  and alert_date <= '"
            + Util.GetSafeRequestValue(Request, "end", "2021-12-20") + "' "
            + " and exists ( select 'a' from limit_up b where a.gid = b.gid and  a.alert_date = dbo.func_GetLastTransactDate(b.alert_date, 2) ) "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            */
            ...
            if (Util.GetSafeRequestValue(Request, "vol", "0").Trim().Equals("1"))
            {
                if (s.kLineDay[alertIndex + 2].volume <= s.kLineDay[alertIndex + 1].volume)
                {
                    continue;
                }
            }
