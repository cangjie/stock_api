...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select a.alert_date as alert_date , a.gid as gid from limit_up a "
            ...
            + " and alert_date < dbo.func_GetLastTransactDate(getdate(), 10) "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            bool willLimitUp = false;
            bool willLowThan3Line = false;
            int currentIndex = 0;
            if (s.gid.Trim().Equals("sh600121"))
            {
                string aa = "aa";
            }
            for (int i = alertIndex + 2; i < s.kLineDay.Length && !willLimitUp && !willLowThan3Line; i++)
            {
                if (s.IsLimitUp(i))
                {
                    willLimitUp = true;
