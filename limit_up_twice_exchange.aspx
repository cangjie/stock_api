...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select gid, alert_date from limit_up a  where alert_date = '" + Util.GetLastTransactDate(currentDate, 2).ToShortDateString()
            + "' and  exists ( select 'a' from limit_up b where a.gid = b.gid and alert_date = '" + Util.GetLastTransactDate(currentDate, 1).ToShortDateString()  + "'  ) ");
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if ((stock.kLineDay[currentIndex - 2].endPrice - stock.kLineDay[currentIndex - 3].endPrice) / stock.kLineDay[currentIndex - 3].endPrice < 0.0995)
            {
                continue;
            }

            if ((stock.kLineDay[currentIndex - 1].endPrice - stock.kLineDay[currentIndex - 2].endPrice) / stock.kLineDay[currentIndex - 2].endPrice < 0.0985)
            {
                continue;
            }
            ...
            if ((stock.kLineDay[currentIndex].endPrice - stock.kLineDay[currentIndex - 1].endPrice) / stock.kLineDay[currentIndex - 1].endPrice > 0.0995)
            {
                continue;
            }
            ...
            int limitUpTimes = 2;
            for (int lc = currentIndex - 3; lc > 0
                && (stock.kLineDay[lc].endPrice - stock.kLineDay[lc - 1].endPrice) / stock.kLineDay[lc - 1].endPrice > 0.095; lc--)
            {
                limitUpTimes++;
            }
            dr["连板"] = limitUpTimes;
