...
<script runat="server">
    ...
    public DataTable RenderHtml(DataRow[] drArr)
    {
        ...
        foreach (DataRow drOri in drArr)
        {
            ...
            dr["名称"] = "<a href=\"io_volume_detail.aspx?gid=" + gid.Trim() + "\" target=\"_blank\" >" + dr["名称"].ToString() + "</a>";
            dt.Rows.Add(dr);
        }
        AddTotal(drArr, dt);
        return dt;
    ...
    public static DataTable GetData(DateTime currentDate, string days)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            int highestIndex = GetNearlyHighestIndex(stock.kLineDay, currentIndex, daysAbove3Line + 1);
    ...
    public static int GetNearlyHighestIndex(KLine[] kArr, int startIndex, int count)
    {
        int highestIndex = startIndex;
        double highestPrice = 0;
        for (int i = startIndex; i > startIndex - count; i--)
        {
            if (kArr[i].highestPrice > highestPrice)
            {
                highestPrice = kArr[i].highestPrice;
                highestIndex = i;
            }
        }
        return highestIndex;
