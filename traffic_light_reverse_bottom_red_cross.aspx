...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable("select * from alert_traffic_light where alert_date >= '"
            + Util.GetLastTransactDate(currentDate, 20).ToShortDateString() + "' and alert_date <= '"
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (trafficLightIndex < 1 || trafficLightIndex + 1 >= stock.kLineDay.Length)
            {
                continue;
            }
            ...
            if (stock.kLineDay[currentIndex].endPrice < stock.kLineDay[currentIndex - 1].endPrice)
            {
                continue;
            }

            if ((stock.kLineDay[currentIndex - 1].endPrice - stock.kLineDay[currentIndex - 2].endPrice) / stock.kLineDay[currentIndex - 2].endPrice >= -0.02)
            {
                continue;
            }

            int minPriceIndex;
            double minPrice = GetFirstLowestPrice(stock.kLineDay, trafficLightIndex, out minPriceIndex);
            ...
            for (int i = trafficLightIndex; i < stock.kLineDay.Length && stock.kLineDay[i].endPrice >= stock.GetAverageSettlePrice(i, 3, 3) ; i++)
            {
                if (maxPrice < stock.kLineDay[i].highestPrice)
                {
                    maxPrice = stock.kLineDay[i].highestPrice;
                    maxPriceIndex = i;
                }
            }
            ...
            for (int i = trafficLightIndex + 2; i < trafficLightIndex + 20 && i < stock.kLineDay.Length; i++)
            {
                if (stock.kLineDay[i - 1].endPrice < f4 && (stock.kLineDay[i - 1].endPrice - stock.kLineDay[i - 2].endPrice) / stock.kLineDay[i - 2].endPrice <= -0.02
                    && (stock.kLineDay[i].startPrice < stock.kLineDay[i].endPrice ) && stock.kLineDay[i].volume < stock.kLineDay[i-1].volume)
                {
                    buyIndex = i;
                    break;
                }
            ...
            dr["红绿灯涨"] = (trafficLightPrice - maxPrice) / maxPrice;
            ...
            for (int i = 1; i + currentIndex < stock.kLineDay.Length && !willCross3Line; i++)
            {
                if (stock.kLineDay[currentIndex + i - 1].endPrice <= stock.GetAverageSettlePrice(currentIndex + i - 1, 3, 3)
                    && stock.kLineDay[currentIndex + i].endPrice >= stock.GetAverageSettlePrice(currentIndex + i, 3, 3))
                {
                    willCross3Line = true;
