...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (s.kLineDay[alertIndex].startPrice == s.kLineDay[alertIndex].endPrice)
            {
                isMinus = true;
            }


            if (Math.Min(s.kLineDay[alertIndex + 1].endPrice, s.kLineDay[alertIndex + 1].startPrice) < s.kLineDay[alertIndex].highestPrice)
            {
                continue;
            }

            if (Math.Max(s.kLineDay[alertIndex + 1].endPrice, s.kLineDay[alertIndex + 1].startPrice) < s.kLineDay[alertIndex + 1].highestPrice)
            {
                continue;
            }
            ...
            double limitUpPrice = s.kLineDay[alertIndex].highestPrice;
            if ((s.kLineDay[alertIndex + 1].highestPrice - limitUpPrice) / limitUpPrice >= 0.1)
            {
                continue;
            }
...
<body>
    <form id="form1" runat="server">
        ...
        <div>1字板</div>
        <div>
            总计：<%=count1.ToString() %> / <%=Math.Round((double)100*suc1/(double)count1, 2).ToString() %>%<br />
            涨5%：<%=newHighCount1.ToString() %> / <%=Math.Round((double)100*newHighCount1/(double)count1, 2).ToString() %>%
        </div>
        <div>
