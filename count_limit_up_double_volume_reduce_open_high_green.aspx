...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select alert_date, gid  from limit_up a  where  "
            ...
            + " and not exists ( select 'a' from limit_up b where a.gid = b.gid and b.alert_date = dbo.func_getlasttransactdate(a.alert_date, 3)  ) "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (alertIndex < 2 || alertIndex >= s.kLineDay.Length - 2 - days)
            {
                continue;
            }
            ...
            if (s.kLineDay[buyIndex].startPrice < s.kLineDay[alertIndex].highestPrice)
            {
                continue;
            }

            if (s.kLineDay[buyIndex].startPrice >= s.kLineDay[alertIndex].endPrice * 1.095)
            {
                continue;
            }
            ...
            for (int i = 1; i <= 20 && alertIndex - i >= 0; i++)
            {
                if (s.kLineDay[alertIndex - i].highestPrice > s.kLineDay[alertIndex].highestPrice)
                {
                    newHigh = false;
                    break;
                }
