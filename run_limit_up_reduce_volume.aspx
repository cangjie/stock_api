...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if ((stock.kLineDay[limitUpIndex + 1].endPrice - stock.kLineDay[limitUpIndex].endPrice) / stock.kLineDay[limitUpIndex].endPrice < -0.095)
            {
                continue;
            }
            ...
            for (int i = limitUpIndex - 1; i >= 0 && stock.kLineDay[i].endPrice >= stock.GetAverageSettlePrice(i, 3, 3); i--)
            {
                ...
                {
                    limitUpNum++;
                    minVolume = Math.Min(minVolume, stock.kLineDay[i].volume);
                }
            }
            if (firstVolume >= minVolume)
            {
                continue;
            }

            double buyPrice = stock.kLineDay[limitUpIndex + 1].endPrice;
            int currentIndex = limitUpIndex;
            DataRow dr = dt.NewRow();
            dr["日期"] = stock.kLineDay[limitUpIndex + 1].startDateTime.ToShortDateString();
            dr["代码"] = stock.gid;
