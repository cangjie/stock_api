...
<script runat="server">
    ...
    public DataTable GetData(string countPage)
    {
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            bool existsNewHighSettle = false;

            double prevHighPrice = Math.Max(s.kLineDay[alertIndex].highestPrice, s.kLineDay[alertIndex + 1].highestPrice);

            for (int i = 1; i <= 5 && alertIndex + 1 + i < s.kLineDay.Length; i++)
            {
                if (s.kLineDay[alertIndex + 1 + i].endPrice > prevHighPrice)
                {
                    existsNewHighSettle = true;
                    break;
                }
            }

            if (!existsNewHighSettle)
            {
                continue;
            }
            ...
            for (int i = alertIndex - 1 ; i < s.kLineDay.Length && s.kLineDay[i].endPrice >= s.GetAverageSettlePrice(i, 3, 3); i++)
            {
