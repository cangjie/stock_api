...
<script runat="server">
    ...
    public  DataTable GetData(DateTime currentDate, int days)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable("select * from limit_up a where exists (select 'a' from limit_up b where a.gid = b.gid and a.alert_date = dbo.func_GetNextTransactDate(b.alert_date, 1)) "
            + " and exists (select 'a' from limit_up c where a.gid = c.gid and a.alert_date = dbo.func_GetNextTransactDate(c.alert_date, 2)) "
            + " and  a.alert_date >= '" + Util.GetLastTransactDate(currentDate, 20).ToShortDateString() + "' "
            + " and a.alert_date < '" + Util.GetLastTransactDate(currentDate, 1).ToShortDateString() + "'  "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            int lowestIndex = -1;
            GetFirstLowestPrice(stock.kLineDay, alertIndex, out lowestIndex);
            int highestIndex = stock.GetItemIndex(alertDate);
            double highestPrice = stock.kLineDay[highestIndex].highestPrice;
            for (int i = highestIndex + 1; i <= currentIndex && stock.kLineDay[i].endPrice >= stock.GetAverageSettlePrice(i, 3, 3); i++)
            {
            ...
            for (int i = lowestIndex+2; i <= highestIndex; i++)
            {
                if (stock.IsLimitUp(i - 2) && stock.IsLimitUp(i - 1) && stock.IsLimitUp(i))
                {
                    triLimit = true;
                    break;
                }
