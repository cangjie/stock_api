...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        DateTime endDate = DateTime.Parse(Util.GetSafeRequestValue(Request, "end", "2022-6-1"));
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (limitUpDayIndex < 1)
            {
                continue;
            }

            int buyIndex = limitUpDayIndex + 1;
            if (buyIndex > s.kLineDay.Length - 2)
            {
                continue;
            }
            ...
            if (s.kLineDay[buyIndex].highestPrice > Math.Max(s.kLineDay[buyIndex].endPrice, s.kLineDay[buyIndex].startPrice))
            {
                continue;
            }

            if (s.kLineDay[limitUpDayIndex].highestPrice > Math.Min(s.kLineDay[buyIndex].startPrice, s.kLineDay[buyIndex].endPrice))
            {
                continue;
            }
            int diff = (int)(100 * Math.Abs(s.kLineDay[buyIndex].volume - s.kLineDay[buyIndex - 1].volume) / s.kLineDay[buyIndex - 1].volume);
            if (diff > volumeDiff)
            {
                continue;
            }
...
<body>
    <form id="form1" runat="server">
        <div>
            1%：<%= Math.Round(100 * (double)sucCount / (double)totalCount, 2).ToString() %>%<br />
            5%：<%= Math.Round(100 * (double)bigSuc / (double)totalCount, 2).ToString() %>%
        </div>
        <div>
