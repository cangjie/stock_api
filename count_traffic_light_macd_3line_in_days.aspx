...
<script runat="server">
    ...
    public DataTable GetData()
    {
        ...
        DateTime endDate = DateTime.Parse(Util.GetSafeRequestValue(Request, "enddate", "2021-1-15"));
        int days = int.Parse(Util.GetSafeRequestValue(Request, "days", "5"));
        int noCoverDays = int.Parse(Util.GetSafeRequestValue(Request, "nocoverdays", "15"));
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select  * from alert_traffic_light where "
            + "  alert_date >= '" + startDate.ToShortDateString()
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            for (int i = alertIndex + 1;  i  < s.kLineDay.Length
                && s.kLineDay[i].highestPrice < s.kLineDay[alertIndex].endPrice; i++)
            {

                if (i - alertIndex > noCoverDays && s.macdDays(i) == 0 && s.kLineDay[i - 1].endPrice < s.GetAverageSettlePrice(i - 1, 3, 3)
                    && s.kLineDay[i].endPrice > s.GetAverageSettlePrice(i, 3, 3))
                {
                    ...
                    if (dt.Select("日期 = '" + s.kLineDay[buyIndex].endDateTime.ToShortDateString() + "' and 代码 = '" + s.gid.Trim() + "'").Length > 0)
                    {
                        continue;
                    }
