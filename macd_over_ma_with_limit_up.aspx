...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select * from alert_predict_macd where exists (select 'a' from limit_up where limit_up.gid = alert_predict_macd.gid and alert_date >= '" + Util.GetLastTransactDate(currentDate, 10).ToShortDateString() + "' ) " 
            + " and valid = 1 and alert_date = '" + Util.GetLastTransactDate(currentDate, 1).ToShortDateString() + "' ");
            ...
            if (Util.IsTransacDay(currentDate) && Util.IsTransacTime(DateTime.Now))
            {
                ...
                */
                    if (dr["信号"].ToString().IndexOf("🛍️") >= 0 && dr["信号"].ToString().IndexOf("🔺") < 0
                        && dr["信号"].ToString().IndexOf("🌟") >= 0 && double.Parse(dr["放量"].ToString()) >= 1.5)
                    {
