...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate, double coverRate, double startLightRate, double endLightRate)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select alert_traffic_light.alert_date, alert_traffic_light.gid from alert_traffic_light  "
            + "  left join alert_macd on alert_traffic_light.gid = alert_macd.gid and alert_traffic_light.alert_date < alert_macd.alert_time and alert_traffic_light.alert_date > dbo.func_GetLastTransactDate(alert_macd.alert_time, 20)  "
            + " where alert_macd.alert_time > '" + currentDate.ToShortDateString() + "' and alert_time < '" + currentDate.ToShortDateString() + " 23:00' "
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            for (int i = trafficLightIndex; i < stock.kLineDay.Length && i < currentIndex; i++)
            {
                if (cross3LineTimes == 0 && stock.kLineDay[i].endPrice < stock.GetAverageSettlePrice(i, 3, 3))
                {
                    cross3LineTimes++;
                }
                if (cross3LineTimes == 1 && stock.kLineDay[i].endPrice > stock.GetAverageSettlePrice(i, 3, 3))
                {
                    cross3LineTimes++;
                }
                if (cross3LineTimes == 2 && stock.kLineDay[i].endPrice < stock.GetAverageSettlePrice(i, 3, 3))
                {
                    cross3LineTimes++;
                    break;
                }
