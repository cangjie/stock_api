...
<script runat="server">
        ...
        foreach (DataRow drOri in drArr)
        {
            ...
            for (int i = 0; i < drArr[0].Table.Columns.Count; i++)
            {
                ...
                if (drArr[0].Table.Columns[i].DataType.FullName.ToString().Equals("System.Double"))
                {
                    ...
                    {
                        case "差值":
                            double val = Math.Round(double.Parse(drOri[i].ToString()), 4);
                            if (val == 0)
                            {
                                dr[i] = "<font coloe=\"red\" >0.00%</font>";
                            }
                            else
                            {
                                dr[i] = Math.Round(100 * val, 2).ToString() + "%";
                            }
                            break;
                        case "综指":
                        case "昨收":
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select gid, alert_date from limit_up where alert_date = '"  + lastTransactDate.ToShortDateString() + "' ");
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (!limitUp2 && ((stock.kLineDay[limitUpIndex+1].endPrice - stock.kLineDay[limitUpIndex].endPrice) / stock.kLineDay[limitUpIndex].endPrice < 0.05
            ...
            if (!limitUp2)
            {
                if (Math.Abs(stock.kLineDay[currentIndex].lowestPrice - stock.kLineDay[limitUpIndex + 1].lowestPrice) / stock.kLineDay[limitUpIndex + 1].lowestPrice > 0.01
                    && Math.Abs(stock.kLineDay[currentIndex].lowestPrice - stock.kLineDay[limitUpIndex + 1].startPrice) / stock.kLineDay[limitUpIndex + 1].startPrice > 0.01)
                {
                    continue;
                }
            }
            ...
            {
                if (Math.Abs(stock.kLineDay[currentIndex].lowestPrice - stock.kLineDay[limitUpIndex].lowestPrice) / stock.kLineDay[limitUpIndex].lowestPrice > 0.01
                    && Math.Abs(stock.kLineDay[currentIndex].lowestPrice - stock.kLineDay[limitUpIndex].highestPrice) / stock.kLineDay[limitUpIndex].highestPrice > 0.01
                    && Math.Abs(stock.kLineDay[currentIndex].lowestPrice - stock.kLineDay[limitUpIndex].startPrice) / stock.kLineDay[limitUpIndex].startPrice > 0.01
                    && Math.Abs(stock.kLineDay[currentIndex].lowestPrice - stock.kLineDay[limitUpIndex + 1].lowestPrice) / stock.kLineDay[limitUpIndex + 1].lowestPrice > 0.01
                    && Math.Abs(stock.kLineDay[currentIndex].lowestPrice - stock.kLineDay[limitUpIndex + 1].startPrice) / stock.kLineDay[limitUpIndex + 1].startPrice > 0.01)
                {
                    continue;
                }
