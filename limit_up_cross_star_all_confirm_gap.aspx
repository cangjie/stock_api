...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        DateTime limitUpStartDate = Util.GetLastTransactDate(currentDate, 5);
        DateTime limitUpEndDate = Util.GetLastTransactDate(currentDate, 2);
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            bool noGap = false;
            bool existWBottom = false;
            double lowestOnGap = double.MaxValue;
            for (int i = limitUpIndex + 1; i <= currentIndex; i++)
            {

                if (stock.kLineDay[i].lowestPrice <= stock.kLineDay[limitUpIndex].highestPrice)
                {
                    noGap = true;
                    break;
                }
                else
                {
                    if (i < currentIndex)
                    {
                        lowestOnGap = Math.Min(stock.kLineDay[i].lowestPrice, lowestOnGap);
                    }
                }
            }
            if (noGap)
            ...
            for (int i = limitUpIndex + 1; i < currentIndex; i++)
            {
                maxPriceAfterLimitUp = Math.Max(stock.kLineDay[i].highestPrice, maxPriceAfterLimitUp);
            }

            if (stock.kLineDay[currentIndex].startPrice <= maxPriceAfterLimitUp)
            {
                continue;
            }
