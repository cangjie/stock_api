...
<script runat="server">
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        DataTable dtOri = DBHelper.GetDataTable(" select  * from limit_up_twice where  alert_date = '" + currentDate.ToShortDateString() + "' "
                // + " and gid = 'sz002426' "
                );
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            if (!stock.IsLimitUp(currentIndex) || !stock.IsLimitUp(currentIndex - 1)
                || stock.kLineDay[currentIndex].highestPrice != stock.kLineDay[currentIndex].lowestPrice
                || stock.kLineDay[currentIndex-1].highestPrice != stock.kLineDay[currentIndex-1].lowestPrice)
            {
                continue;
            }
