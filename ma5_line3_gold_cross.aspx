...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        sort = Util.GetSafeRequestValue(Request, "sort", "MACD周,MACD");
        if (!IsPostBack)
        {
    ...
    public static DataTable GetData(DateTime currentDate)
    {
        ...
        */
        ...
        foreach (DataRow drOri in dtOri.Rows)
        {
            ...
            int currentIndex = stock.GetItemIndex(currentDate) - 1;
            ...
            double last5Line = stock.GetAverageSettlePrice(currentIndex - 1, 5, 5);
            double lastMa5 = stock.GetAverageSettlePrice(currentIndex - 1, 5, 0);
            double currentPrice = stock.kLineDay[currentIndex].endPrice;
            double currentStart = stock.kLineDay[currentIndex].startPrice;
            double current3Line = stock.GetAverageSettlePrice(currentIndex, 3, 3);
            double current5Line = stock.GetAverageSettlePrice(currentIndex, 5, 5);
            double currentMa5 = stock.GetAverageSettlePrice(currentIndex, 5, 0);
            double nextPrice = stock.kLineDay[currentIndex + 1].endPrice;
            double nextStart = stock.kLineDay[currentIndex + 1].startPrice;
            double next3Line = stock.GetAverageSettlePrice(currentIndex + 1, 3, 3);
            double next5Line = stock.GetAverageSettlePrice(currentIndex + 1, 5, 5);
            double nextMa5 = stock.GetAverageSettlePrice(currentIndex + 1, 5, 0);
            ...
            double highestPrice = Math.Max(Math.Max(stock.kLineDay[currentIndex - 1].highestPrice,
                stock.kLineDay[currentIndex].highestPrice), stock.kLineDay[currentIndex + 1].highestPrice);
            double lowestPrice = Math.Min(Math.Min(stock.kLineDay[currentIndex - 1].lowestPrice,
                stock.kLineDay[currentIndex].lowestPrice), stock.kLineDay[currentIndex + 1].lowestPrice);
